...
# library(INLA)       # A package for quick Bayesian inference 
...
# y_correlation = 0.8,  # Correlation between Y covariates
...
prepare_nimble_inputs <- function(bookkeeping, adjacency, data) {
  ...
  inits <- list(
    ...
    # Latent variables
    x_latent = matrix(0.01, nrow = D - constants$J_x, ncol = p_x),
    y_latent = rep(0.01, D - constants$J_y),
    yx_latent = matrix(0.01, nrow = D - constants$J_y, ncol = p_y),
  ...
  for(j in 1:p_x) {
    for(m in 1:(S_x-constants$J_x)) {
      total_count <- data$x[constants$J_x + m, j]
      n_atoms <- diff(constants$xlatent_ind[m,]) + 1
      base_count <- floor(total_count / n_atoms)
      remainder <- total_count %% n_atoms
      inits$x_latent[constants$xlatent_ind[m,1]:constants$xlatent_ind[m,2], j] <- base_count
      if(remainder > 0) {
        inits$x_latent[constants$xlatent_ind[m,1]:(constants$xlatent_ind[m,1]+remainder-1), j] <- 
          base_count + 1
      }
    }
  }
  ...
  for(m in 1:(S_y-constants$J_y)) {
    total_count <- data$y[constants$J_y + m, p_y + 1]
    n_atoms <- diff(constants$ylatent_ind[m,]) + 1
    base_count <- floor(total_count / n_atoms)
    remainder <- total_count %% n_atoms
    inits$y_latent[constants$ylatent_ind[m,1]:constants$ylatent_ind[m,2]] <- base_count
    if(remainder > 0) {
      inits$y_latent[constants$ylatent_ind[m,1]:(constants$ylatent_ind[m,1]+remainder-1)] <- 
        base_count + 1
    }
  }
  ...
  for(j in 1:p_y) {
    for(m in 1:(S_y-constants$J_y)) {
      total_count <- data$y[constants$J_y + m, j]
      n_atoms <- diff(constants$ylatent_ind[m,]) + 1
      base_count <- floor(total_count / n_atoms)
      remainder <- total_count %% n_atoms
      inits$yx_latent[constants$ylatent_ind[m,1]:constants$ylatent_ind[m,2], j] <- base_count
      if(remainder > 0) {
        inits$yx_latent[constants$ylatent_ind[m,1]:(constants$ylatent_ind[m,1]+remainder-1), j] <- 
          base_count + 1
      }
    }
  }
  ...
  validate_latent_counts <- function(inits, data, constants) {
    # Validate x_latent sums
    for(j in 1:constants$p_x) {
      for(m in 1:(constants$S_x - constants$J_x)) {
        total <- sum(inits$x_latent[constants$xlatent_ind[m,1]:constants$xlatent_ind[m,2], j])
        observed <- data$x[constants$J_x + m, j]
        if(total != observed) {
          stop(sprintf("x_latent[,%d] sum mismatch for group %d: got %d, expected %d", 
                       j, m, total, observed))
        }
        # Check for non-negative integers
        if(any(inits$x_latent[constants$xlatent_ind[m,1]:constants$xlatent_ind[m,2], j] < 0) ||
           any(inits$x_latent[constants$xlatent_ind[m,1]:constants$xlatent_ind[m,2], j] != 
               floor(inits$x_latent[constants$xlatent_ind[m,1]:constants$xlatent_ind[m,2], j]))) {
          stop(sprintf("x_latent[,%d] contains negative or non-integer values in group %d", j, m))
        }
      }
    }
    
    # Validate y_latent sums
    for(m in 1:(constants$S_y - constants$J_y)) {
      total <- sum(inits$y_latent[constants$ylatent_ind[m,1]:constants$ylatent_ind[m,2]])
      observed <- data$y[constants$J_y + m, constants$p_y + 1]
      if(total != observed) {
        stop(sprintf("y_latent sum mismatch for group %d: got %d, expected %d", 
                     m, total, observed))
      }
      # Check for non-negative integers
      if(any(inits$y_latent[constants$ylatent_ind[m,1]:constants$ylatent_ind[m,2]] < 0) ||
         any(inits$y_latent[constants$ylatent_ind[m,1]:constants$ylatent_ind[m,2]] != 
             floor(inits$y_latent[constants$ylatent_ind[m,1]:constants$ylatent_ind[m,2]]))) {
        stop(sprintf("y_latent contains negative or non-integer values in group %d", m))
      }
    }
    
    # Validate yx_latent sums
    for(j in 1:constants$p_y) {
      for(m in 1:(constants$S_y - constants$J_y)) {
        total <- sum(inits$yx_latent[constants$ylatent_ind[m,1]:constants$ylatent_ind[m,2], j])
        observed <- data$y[constants$J_y + m, j]
        if(total != observed) {
          stop(sprintf("yx_latent[,%d] sum mismatch for group %d: got %d, expected %d", 
                       j, m, total, observed))
        }
        # Check for non-negative integers
        if(any(inits$yx_latent[constants$ylatent_ind[m,1]:constants$ylatent_ind[m,2], j] < 0) ||
           any(inits$yx_latent[constants$ylatent_ind[m,1]:constants$ylatent_ind[m,2], j] != 
               floor(inits$yx_latent[constants$ylatent_ind[m,1]:constants$ylatent_ind[m,2], j]))) {
          stop(sprintf("yx_latent[,%d] contains negative or non-integer values in group %d", j, m))
        }
      }
    }
    
    # Add informative output about successful validation
    cat("\nLatent variable validation summary:")
    cat("\n- Validated", constants$p_x, "x_latent variables across", (constants$S_x - constants$J_x), "groups")
    cat("\n- Validated y_latent variable across", (constants$S_y - constants$J_y), "groups")
    cat("\n- Validated", constants$p_y, "yx_latent variables across", (constants$S_y - constants$J_y), "groups")
    cat("\nAll latent counts match observed totals and are valid non-negative integers.\n")
    
    return(TRUE)
  }
  
  check_probabilities <- function(inits, constants) {
    # Check that probabilities will be valid
    eps <- 1e-10
    for(d in 1:constants$D) {
      if(any(inits$lambda_atom_x[d,] <= eps) || 
         any(inits$lambda_atom_yx[d,] <= eps) || 
         inits$lambda_atom_y[d] <= eps) {
        warning("Very small lambda values detected")
      }
    }
  }
  
  # Validate results before return
  validate_latent_counts(inits, data, constants)
  check_probabilities(inits, constants)
...
run_nimble_model <- function(constants, data, inits, niter = 10000, nburnin = 5000, 
                             nchains = 3) {
  source('nimble_abrm_0115.R')
  ...
  mcmcConf$addSampler(target = beta_nodes,
                      ...
                      control = list(adaptive = TRUE,
                                     ...
                                     adaptInterval = 500, # More frequent adaptation
                                     adaptFactorExponent = 0.8)) # More conservative adaptation
