...
abrm <- nimbleCode({
  ...
  for (j in 1:p_x){ 
    ...
    psi_x[1:S_x,j] ~ dcar_normal(adj_x[], weights_x[], num_x[], tau_x[j], zero_mean = 1)
  }
  
  ## this part allows for correlation in the spatial random effects across x-grid covariates
  Prec_x[1:p_x,1:p_x] ~ dwish(diag(p_x),p_x)
  ...
  for (j in 1:p_y){ 
    ...
    psi_yx[1:S_y,j] ~ dcar_normal(adj_y[], weights_y[], num_y[], tau_yx[j], zero_mean = 1)
  }
  
  ## this part allows for correlation in the spatial random effects across y-grid covariates
  Prec_yx[1:p_y,1:p_y] ~ dwish(diag(p_y),p_y)
